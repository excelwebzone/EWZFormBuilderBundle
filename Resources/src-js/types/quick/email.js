...
FormBuilder.EmailType = FormBuilder.Type.extend({
    ...
    init: function() {
        ...
        var template = '<@ if (sublabel) { @><div class="form-sub-label-container"><@ } @><input type="email" name="<@=id@>" id="field_<@=id@>" size="<@=size@>" <@ if (maxsize) { @>maxlength="<@=maxsize@>"<@ } @> <@ if (required) { @>required="required"<@ } @> <@ if (hint) { @>placeholder="<@=hint@>"<@ } @> value="<@=value@>" <@ if (readonly) { @>readonly="readonly"<@ } @> class="form-textbox" /><@ if (sublabel) { @><span class="form-sub-label"><@=sublabel@></span></div><@ } @>';
